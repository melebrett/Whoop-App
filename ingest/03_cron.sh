...
SVC_URL=$(gcloud run services describe ingest-whoop-daily --format 'value(status.url)')
echo $SVC_URL
echo $SVC_EMAIL
...
gcloud scheduler jobs create http dailyupdate \
       --description "Ingest whoop using Cloud Run" \
